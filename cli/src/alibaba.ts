...
import type { ApplyFormSchema as _ApplyFormSchema_alibaba } from "./apply.js";
...
export async function fetchApplicationSchema(postId: string): Promise<
  { ok: true; schema: _ApplyFormSchema_alibaba } | { ok: false; source: string; message: string }
> {
  const id = (postId ?? "").trim();
  if (!id) return { ok: false, source: "campus-talent.alibaba.com", message: "post_id is required" };
  let title = "";
